...
class ShiftScheduler:
    ...
    def caluculate_days(self):
        #第一月曜日の日付返す
        day_of_week, number_of_days = calendar.monthrange(self.year, self.month)
        first_monday = 1 if day_of_week == 0 else 8 - day_of_week
        first_monday = first_monday -1
        days_in_month = len(self.days) -1
        weeks_in_month = math.floor((days_in_month - first_monday+1 ) / 7)
        print(first_monday,days_in_month,weeks_in_month)
        return first_monday,days_in_month,weeks_in_month
    ...
    def add_constraint_for_weekly_working_days(self):
        first_monday, days_in_month, weeks_in_month = self.caluculate_days()
        days_in_last_week = (days_in_month + 1) - ((first_monday) + weeks_in_month * 7)
        for employee in self.employees:
    ...
    def add_constraint_for_no_consecutive_work_days(self):
        for employee in self.employees:
            for start_day in range(len(self.days) - 4):  
                consecutive_days_work = pulp.lpSum(employee.assigned_vars[day] for day in range(start_day, start_day + 5))
                paid_off = pulp.lpSum(employee.paid_vars[day] for day in range(start_day, start_day + 5))
                self.prob += consecutive_days_work + paid_off <= 4, f"no_consecutive_5_days_work_{employee.id}_{start_day}"
    ...
    def add_constraint_for_all_employees_third_thursday_attendance(self):
        ...
        if first_monday >= 4:
            the_third_Thursday = first_monday + 10
        else:
            the_third_Thursday = first_monday + 17
        ...
        self.prob += pulp.lpSum(employee.shift_vars[(the_third_Thursday, self.work_types[5])]
                               for employee in self.employees) >= 1
        for employee in self.employees:
                self.prob += pulp.lpSum(employee.shift_vars[(the_third_Thursday, work_type)] for work_type in self.work_types) == 1
        ...
        for employee in self.employees:
            ...
            if employee.id == 1:
                ...
                for start_day in range(len(self.days) - 2):  
                    consecutive_days_work = pulp.lpSum(employee.assigned_vars[day] for day in range(start_day, start_day + 3))
                    paid_off = pulp.lpSum(employee.paid_vars[day] for day in range(start_day, start_day + 3))
                    self.prob += consecutive_days_work+paid_off <= 2, f"no_consecutive_3_days_work_{employee.id}_{start_day}"
                #夕食の次の日昼食は嫌
            ...
            if employee.id == 5:
                ...
                for day in range(len(self.days)):
                    if day%7 == (first_monday+5)%7:
                        holidays.append(day)
                    elif day%7 == (first_monday+6)%7:
                        holidays.append(day)
                    else:
                        continue
        ...
        if first_monday != 0:
            ...
            for employee in self.employees:
                if not employee.over_work :
                    ...
                    self.prob += employee.weekly_days - employee.last_month_period_work_days -1 <= pulp.lpSum(employee.shift_vars[(day, work_type)] 
                                                for day in range(first_monday)
                                                for work_type in self.work_types)
    ...
    def add_constraint_for_no_consecutive_work_days_informed_by_last_month_shift(self):
        for employee in self.employees:
            consecutive_days_work = pulp.lpSum(employee.assigned_vars[day] for day in range(5- employee.last_month_consecutive_days))
    ...
    def add_constraint_for_individual_employee_informed_by_last_month_shift(self):
        for employee in self.employees:
            ...
            if employee.id == 1:
                #３日連続勤務の禁止
                consecutive_days_work = pulp.lpSum(employee.assigned_vars[day] for day in range(3 - employee.last_month_consecutive_days))
    ...
    def add_constraint_for_holiday_count_for_full_time_emp(self):
        for full_emp in self.full_time_employees:
            self.prob += pulp.lpSum(full_emp.shift_vars[day] for day in self.days) == len(self.days) - full_emp.holiday_count_per_month
    ...
    def add_constraint_for_no_consecutive_work_days_for_full_time_emp(self):
        for full_emp in self.full_time_employees:
            for start_day in range(len(self.days) - 4):  
                ...
                self.prob += consecutive_days_work + paid_off <= 4, f"no_consecutive_5_days_work_{full_emp.id}_{start_day}"
    ...
    def add_constraint_for_no_consecutive_work_days_informed_by_last_month_shift_for_full_time_emp(self):
        for full_emp in self.full_time_employees:
            consecutive_days_work = pulp.lpSum(full_emp.shift_vars[day] for day in range(5- full_emp.last_month_consecutive_days))
