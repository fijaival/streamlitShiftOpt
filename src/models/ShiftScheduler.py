...
class ShiftScheduler:
    ...
    def caluculate_days(self):
        # 第一月曜日の日付返す
        day_of_week, number_of_days = calendar.monthrange(
        ...
        for employee in self.employees:
            for week in range(weeks_in_month):
                ...
                if employee.over_work:
                    ...
                    self.prob += (over_work_condition-2) / \
                        10 <= employee.penalty_vars[week]
    ...
    def add_constraint_for_minimum_workers_per_day(self):
        for day in self.days:
            ...
            self.prob += temp_emp + full_emp >= self.absolute_min_workers_per_day + \
                1, f"Min_workers_day_{day}"
    ...
    def add_constraint_for_no_work_on_paid_off(self):
        for employee in self.employees:
            ...
            for day in self.days:
                self.prob += employee.assigned_vars[day] + \
                    employee.paid_vars[day] <= 1
    ...
    def add_constraint_for_all_employees_third_thursday_attendance(self):
        ...
        self.prob += pulp.lpSum(employee.shift_vars[(the_third_Thursday, self.work_types[5])]
                                for employee in self.employees) >= 1
        for employee in self.employees:
            self.prob += pulp.lpSum(employee.shift_vars[(
                the_third_Thursday, work_type)] for work_type in self.work_types) == 1
    ...
    def add_constraint_for_no_consecutive_work_days_informed_by_last_month_shift(self):
        for employee in self.employees:
            consecutive_days_work = pulp.lpSum(employee.assigned_vars[day] for day in range(
                5 - employee.last_month_consecutive_days))
            paid_off = pulp.lpSum(employee.paid_vars[day] for day in range(
                5 - employee.last_month_consecutive_days))
            self.prob += consecutive_days_work + paid_off <= 4 - \
                employee.last_month_consecutive_days
        ...
        for employee in self.employees:
            ...
            if employee.id == 1:
                ...
                if employee.last_month_consecutive_days >= 1:
                    self.prob += employee.shift_vars[(0,
                                                      self.work_types[1])] <= 0
    ...
    def add_constraint_for_holiday_count_for_full_time_emp(self):
        for full_emp in self.full_time_employees:
            self.prob += pulp.lpSum(full_emp.shift_vars[day] for day in self.days) == len(
                self.days) - full_emp.holiday_count_per_month
    ...
    def add_constraint_for_paid_off_for_full_time_emp(self):
        for full_emp in self.full_time_employees:
            ...
            for day in self.days:
                self.prob += full_emp.shift_vars[day] + \
                    full_emp.paid_vars[day] <= 1
    ...
    def add_constraint_for_no_consecutive_work_days_for_full_time_emp(self):
        for full_emp in self.full_time_employees:
            for start_day in range(len(self.days) - 4):
                consecutive_days_work = pulp.lpSum(
                    full_emp.shift_vars[day] for day in range(start_day, start_day + 5))
                paid_off = pulp.lpSum(
    ...
    def add_constraint_for_no_consecutive_work_days_informed_by_last_month_shift_for_full_time_emp(self):
        for full_emp in self.full_time_employees:
            consecutive_days_work = pulp.lpSum(full_emp.shift_vars[day] for day in range(
                5 - full_emp.last_month_consecutive_days))
            paid_off = pulp.lpSum(full_emp.paid_vars[day] for day in range(
                5 - full_emp.last_month_consecutive_days))
            self.prob += consecutive_days_work + paid_off <= 4 - \
                full_emp.last_month_consecutive_days
    ...
    def create_employees_schedule(self):
        ...
        df_weekday = pd.DataFrame([weekday_data], index=[
                                  '曜日'], columns=column_names)
